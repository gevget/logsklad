...
export function WarehouseIntakeForm({ orderId, expectedPlaces, expectedWeight, preview = false }: {
  ...
  return <form action={recordWarehouseIntakeAction} className="warehouse-intake-form">
    ...
    <div className="warehouse-intake-values" role="group" aria-label="Сравнение ожидаемого и фактического груза">
    ...
    <div className="warehouse-intake-field-group">
      <div className="warehouse-intake-section-heading"><span>ДАННЫЕ ПРИЁМКИ</span><strong>Фактические показатели</strong></div>
      <div className="warehouse-intake-inputs" role="group" aria-label="Фактические количество и вес">
    ...
    <label className="warehouse-release-photo"><Camera size={16} /><span>Фото приёмки · необязательно</span><input type="file" name="photo" accept="image/jpeg,image/png,image/webp" capture="environment" disabled={preview} onChange={(event) => setPhotoName(event.currentTarget.files?.[0]?.name ?? "")} /></label>
    {photoName ? <span className="warehouse-file-selection" role="status" aria-live="polite">{photoName}</span> : null}
...
export function WarehouseReleaseForm({
  ...
  return <>
    ...
    <form action={recordWarehouseReleaseAction} className="warehouse-intake-form warehouse-release-form">
      ...
      <label className="warehouse-release-photo"><Camera size={16} /><span>Фото складской выдачи · необязательно</span><input type="file" name="photo" accept="image/jpeg,image/png,image/webp" capture="environment" disabled={preview || !assignmentReady} onChange={(event) => setPhotoName(event.currentTarget.files?.[0]?.name ?? "")} /></label>
      {photoName ? <span className="warehouse-file-selection" role="status" aria-live="polite">{photoName}</span> : null}
